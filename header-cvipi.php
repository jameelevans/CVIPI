...
$posts_header_style = ! empty( $posts_header_data['image'] )
	? ' style="--cvipi-hero-image: url(\'' . esc_url( $posts_header_data['image'] ) . '\');"'
	: '';
?>
<!doctype html>
	...
	<section class="cvipi-hero">
		...
		<div class="cvipi-hero__content">
			...
			<?php if ( ! empty( $posts_header_data['title'] ) ) : ?>
				<h1 class="cvipi-hero__heading"><?php echo wp_kses_post( $posts_header_data['title'] ); ?></h1>
			<?php endif; ?>

			<?php if ( ! empty( $posts_header_data['description'] ) ) : ?>
				<p class="cvipi-hero__description"><?php echo esc_html( $posts_header_data['description'] ); ?></p>
			<?php endif; ?>
		</div>
	</section>
